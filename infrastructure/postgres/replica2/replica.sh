...
until pg_isready -h postgres2 -p 5432; do
  sleep 2
done
...
PGPASSWORD=strongpassword pg_basebackup -h postgres2 -D "$PGDATA" -U replicator -v -P --wal-method=stream -R -S replica2_slot
